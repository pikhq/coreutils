...
static void cksum(FILE *f, char *name)
{
	...
	while((read_len = fread(buf, 1, sizeof(buf), f)) != 0) {
		sum = update_crc(read_len, buf, sum);
		len += read_len;
	}
	if(ferror(f)) return;

	for(uintmax_t n = len; n != 0; n >>= 8) {
		sum = update_crc(1, &(char){n}, sum);
	}
	
	printf("%"PRIu32" %jd%s%s\n", ~sum, (intmax_t)len, name ? " " : "", name ? name : "");
}
